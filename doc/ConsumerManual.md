...
```
$ curl -v -X POST -H "Cache-Control: no-cache" -H "Content-Type: application/json" -d "consumer_id={利用者ID}" -d "password={利用者のパスワード}" https://key-authen.test.data-linkage.jp/cadde/api/v1/consumer/token
```
