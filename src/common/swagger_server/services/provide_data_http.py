...
def provide_data_http(
    ...
    Raises:
        ...
        Cadde_excption: コンフィグファイルからベーシック認証時のIDもしくはパスワードが取得できない場合 エラーコード: 00002E
        Cadde_excption: ファイルのダウンロードに失敗した場合 エラーコード: 01002E
        Cadde_excption: 参照先URLにファイルが見つからない場合 エラーコード: 01003E
        Cadde_excption: HTTP接続時のベーシック認証に失敗している場合 エラーコード: 01004E
        Cadde_excption: リソースURLからドメイン情報の取得に失敗した場合 エラーコード: 01005E
        Cadde_excption: タイムアウトが発生した場合 エラーコード: 01006E

    """
    if not resource_url:
        raise CaddeException('00001E')

    logger.debug(get_message('01001N', [resource_url, headers_dict]))
    ...
    if response.status_code == requests.codes.not_found:
        raise CaddeException('01003E')

    if response.status_code == requests.codes.unauthorized:
        raise CaddeException('01004E')
